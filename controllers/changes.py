...
class CSVController:
    def __init__(self):
        print("--- CONTROLADOR V36: AIRES INTELIGENTES (TRANSITORIOS + ESTABILIDAD) ---")
    ...
    def _detect_date_format(self, rows, col_idx):
        ...
        sep = '/'
        for row in rows[:10]:
            if col_idx < len(row) and row[col_idx].strip() and '-' in row[col_idx]:
                sep = '-'
                break
        p1_values = set()
        p2_values = set()
    ...
    def get_values_for_device(self, context_key: str, device_name: str, start_times: List[str] = None, end_times: List[str] = None) -> List[Tuple[str, str]]:
        ...
        # 4. AIRES ACONDICIONADOS (LÓGICA INTELIGENTE)
    ...
    # =========================================================================
    #  LÓGICA AIRES ACONDICIONADOS (NUEVO)
    # =========================================================================
    def _generate_ac_profile(self, raw_data, base_date, start_times, end_times):
        """
        Analiza los primeros minutos del CSV para encontrar estabilidad y proyecta.
        """
        if not raw_data: return []
        
        # 1. Analizar transitorio y encontrar valor estable
        numeric_vals = []
        ...
        stable_val = 0.0
        if numeric_vals:
            # Tomamos los primeros 15 minutos o lo que haya
            sample = numeric_vals[:min(len(numeric_vals), 15)]
            # Si hay poca varianza, usamos el promedio
            if len(sample) > 1 and statistics.stdev(sample) < (sum(sample)/len(sample) * 0.2):
                 stable_val = sum(sample) / len(sample)
            else:
                # Si es muy caótico, usamos el máximo del arranque como peor caso (conservador)
                stable_val = max(sample)
        
        stable_val_str = str(stable_val).replace('.', ',')

        # 2. Generar perfil cuadrado con ese valor estable
        timeline = []
        ...
        for i in range(len(start_times)):
            ...
            try:
                ...
                if dt_e < dt_s:
                    ...
                    for p in timeline:
                        if dt_s <= p['dt'] <= dt_end_day: p['val'] = stable_val_str
                        if dt_start_day <= p['dt'] < dt_e: p['val'] = stable_val_str
                else:
                    for p in timeline:
                        if dt_s <= p['dt'] < dt_e: p['val'] = stable_val_str
            except: continue
    ...
    def export_report(self, filename: str, figures: Dict[str, Any] = None, bill_real: float = 0.0):
        ...
        if 'section' in df_summary.columns: df_summary = df_summary.drop(columns=['section'])
