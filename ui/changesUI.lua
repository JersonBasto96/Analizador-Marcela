...
class MainWindow:
    ...
    def _gen_aires_inputs(self, prefix, count, starts=None):
        try: c = int(count)
        except: return
        if not starts: starts = ["00:00"] * c
        frame = getattr(self, f"frame_aires_{prefix}")
        for w in frame.winfo_children(): w.destroy()
        new_starts = []
        for i in range(c):
            f = ttk.Frame(frame)
            ...
            e.pack(side="left")
            new_starts.append(e)
        if prefix == "wd": self.entries_aires_wd_starts = new_starts
        else: self.entries_aires_we_starts = new_starts
    ...
    def _apply_aires_weekly(self):
        ...
        self.controller.set_device_config_weekly('aires', dev, len(wd_s), wd_s, [], len(we_s), we_s, [])
